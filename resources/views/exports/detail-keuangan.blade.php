...
<body>
    ...
    <div class="subheader">
        BULAN
        {{ strtoupper(\Carbon\Carbon::createFromFormat('m', $gajiBruto->bulan_penggajian ?? now()->month)->translatedFormat('F')) }}
        {{ $gajiBruto->tahun_penggajian }}
    </div>

    <table class="info-table">
        <tr>
            <td style="width: 70px;">NO URUT</td>
            <td>: {{ $gajiBruto->user->urutanKeuangan->urutan ?? '-' }}</td>
        </tr>
        <tr>
            <td>N A M A</td>
            <td>: {{ $gajiBruto->user->name ?? '-' }}</td>
        </tr>
    </table>
    ...
    <table class="main-table">
        ...
        <tr>
            ...
            <td class="rp">Rp</td>
            <td class="val">{{ $gajiBruto->nom_jabatan ? number_format($gajiBruto->nom_jabatan, 0, ',', '.') : '-' }}
            </td>
        </tr>
        <tr>
        ...
        <tr>
            <td class="label indent">2 Tunjangan Makan</td>
            <td class="rp">Rp</td>
            <td class="val">{{ $gajiBruto->nom_makan ? number_format($gajiBruto->nom_makan, 0, ',', '.') : '-' }}
            </td>
        </tr>
        <tr>
            <td class="label indent">2 Tunjangan Transport</td>
            <td class="rp">Rp</td>
            <td class="val">
                {{ $gajiBruto->nom_transport ? number_format($gajiBruto->nom_transport, 0, ',', '.') : '-' }}</td>
        </tr>
        <tr>
            <td class="label indent">3 Tunjangan Poskes</td>
            <td class="rp">Rp</td>
            <td class="val">{{ $gajiBruto->nom_poskes ? number_format($gajiBruto->nom_poskes, 0, ',', '.') : '-' }}
            </td>
        </tr>
        <tr>
            ...
            <td class="rp">Rp</td>
            <td class="val">
                {{ $gajiBruto->nom_lainnya ? number_format($gajiBruto->nom_lainnya, 0, ',', '.') : '-' }}</td>
        </tr>
        <tr>
            <td class="label indent">5 Lembur</td>
            <td class="rp">Rp</td>
            <td class="val">{{ $gajiBruto->nom_lembur ? number_format($gajiBruto->nom_lembur, 0, ',', '.') : '-' }}
            </td>
        </tr>
        ...
        <tr>
            ...
            <td class="total-val" style="padding-top: 10px;">
                {{ number_format($gajiBruto->total_bruto ?? 0, 0, ',', '.') }}</td>
        </tr>
    </table>
    ...
    <div class="signature-area">
        <p>Banjarnegara, {{ \Carbon\Carbon::parse($gajiBruto->tanggal_transfer ?? now())->translatedFormat('d F Y') }}
        </p>
        <p>Bendahara,</p>
        <br><br><br>
