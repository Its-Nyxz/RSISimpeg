...
<body>
    ...
    <table class="main-table">
        ...
        <tr>
            ...
            <td class="rp">Rp</td>
            <td class="val">{{ $slip->bruto?->nom_jabatan ? number_format($slip->bruto->nom_jabatan, 0, ',', '.') : '-' }}</td>
        </tr>
        <tr>
            <td class="label indent">2 Tunjangan Fungsional Tambahan</td>
            <td class="rp">Rp</td>
            <td class="val">{{ $slip->bruto?->nom_umum ? number_format($slip->bruto->nom_umum, 0, ',', '.') : '-' }}</td>
        </tr>
        <tr>
            <td class="label indent">3 Tunjangan Poskes</td>
            <td class="rp">Rp</td>
            <td class="val">{{ $slip->bruto?->nom_poskes ? number_format($slip->bruto->nom_poskes, 0, ',', '.') : '-' }}</td>
        </tr>
        <tr>
            <td class="label indent">4 Tunjangan Lainnya</td>
            <td class="rp">Rp</td>
            <td class="val">{{ $slip->bruto?->nom_lainnya ? number_format($slip->bruto->nom_lainnya, 0, ',', '.') : '-' }}</td>
        </tr>
        <tr>
            <td class="label indent">5 Lembur</td>
            <td class="rp">Rp</td>
            <td class="val">{{ $slip->bruto?->nom_lembur ? number_format($slip->bruto->nom_lembur, 0, ',', '.') : '-' }}</td>
        </tr>
        ...
        <tr>
            ...
            <td class="total-val" style="padding-top: 10px;">{{ number_format($slip->bruto?->total_bruto ?? 0, 0, ',', '.') }}</td>
        </tr>
    </table>
    ...
    <table style="width: 100%;">
        <tr class="netto-row">
            ...
            <td class="netto-border" style="text-align: right; width: 120px;">{{ number_format($slip->total_netto ?? 0, 0, ',', '.') }}</td>
        </tr>
    </table>
