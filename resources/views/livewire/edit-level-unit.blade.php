    ...
    <form wire:submit.prevent="updateLevelUnit">
        <div class="grid grid-cols-2 gap-4 bg-success-100 border border-success-200 rounded-lg shadow-lg p-6">
            ...
            <div class="form-group">
                ...
                <div class="relative">
                    <input type="text" id="unit_id" wire:model="unit_id_nama" placeholder="Cari Unit Kerja..."
                        autocomplete="off"
                    ...
                    <ul id="unitDropdown" class="dropdown hidden">
                        @foreach ($unitkerja as $item)
                            <li class="dropdown-item"
                                onclick="selectUnitKerja('{{ $item->nama }}', '{{ $item->id }}')">
                                {{ $item->nama }}
                            </li>
                        @endforeach
                    </ul>
            ...
            <div class="form-group">
                <label for="level_id" class="block text-sm font-medium text-success-900">Level Point</label>
                <div class="relative">
                    <input type="text" id="level_id" wire:model="level_id_nama" placeholder="Cari Level Point..."
                        autocomplete="off"
                        class="form-control mt-1 block w-full rounded-lg border border-gray-300 bg-white focus:ring-success-500 focus:border-success-500 p-2.5"
                    ...
                    <ul id="levelDropdown" class="dropdown hidden">
                        @foreach ($levelpoint as $item)
                            <li class="dropdown-item"
                                onclick="selectLevelPoint('{{ $item->nama }} - {{ $item->point }}', '{{ $item->id }}')">
