<div>
    ...
    @forelse ($users as $user)
        <tr class="odd:bg-success-50 even:bg-success-100 border-b border-success-300 hover:bg-success-300">
            ...
            <td class="px-6 py-4">{{ $user->no_ktp ?? '-'}}</td>
            <td class="px-6 py-4">{{ $user->alamat ?? '-'}}</td>
            <td class="px-6 py-4">{{ $user->jabatan->nama ?? '-' }}</td>
            <td class="px-6 py-4">{{ $user->unitKerja->nama ?? '-' }}</td>
            <td class="px-6 py-4">
            <a href="{{ route('detailkaryawan.show', ['detailkaryawan' => $user->id]) }}" class="text-success-900 px-3 py-2 rounded-md border hover:bg-slate-300">
    ...
    <nav class="inline-flex shadow rounded-md" aria-label="Pagination">
        ...
        @else
            <button wire:click="previousPage" class="px-3 py-2 border rounded-l-md bg-success-600 text-white hover:bg-success-700">
                <i class="fas fa-chevron-left"></i>
            </button>
        @endif
        ...
        @foreach ($users->links()->elements as $element)
            @if (is_array($element))
                @foreach ($element as $page => $url)
                    @if ($page == $users->currentPage() || $page == 1 || $page == $users->lastPage() || abs($page - $users->currentPage()) <= 1)
                        @if ($page == $users->currentPage())
                            <span class="px-3 py-2 border bg-success-600 text-white">{{ $page }}</span>
                        @else
                            <button wire:click="gotoPage({{ $page }})" class="px-3 py-2 border bg-white text-success-600 hover:bg-success-200">
                                {{ $page }}
                            </button>
                        @endif
        ...
        @if ($users->hasMorePages())
            <button wire:click="nextPage" class="px-3 py-2 border rounded-r-md bg-success-600 text-white hover:bg-success-700">
