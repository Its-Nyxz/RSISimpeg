<div>
    <div class="flex justify-between items-center mb-5">
        <h1 class="text-2xl font-bold text-success-900">Tambah Data Level Unit</h1>
        <a href="{{ route('levelunit.index') }}"
    ...
    <form wire:submit.prevent="store">
        <div class="grid grid-cols-2 gap-4 bg-success-100 border border-success-200 rounded-lg shadow-lg p-6">
            ...
            <!-- Level Point -->
            <div class="col-span-2 relative">
                <label for="levelpoint" class="block text-sm font-medium text-success-900">Level Point</label>
                ...
                @if (!empty($levelPointOptions))
                    <ul class="absolute w-full bg-white shadow-md rounded-lg mt-1 overflow-hidden z-10">
                        @foreach ($levelPointOptions as $level)
                            <li class="p-2 hover:bg-success-200 cursor-pointer"
                                wire:click="selectLevelPoint('{{ $level->id }}', '{{ $level->nama }}')">
                                {{ $level->nama }}
                            </li>
                        @endforeach
                    </ul>
