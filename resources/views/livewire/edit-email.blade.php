        ...
        <div class="grid grid-cols-2 gap-4 bg-green-100 border border-green-200 rounded-lg shadow-lg p-6">
            <div class="form-group col-span-2">
                ...
                <div class="grid grid-cols-2 gap-4 items-center">
                    <p class="text-green-700 font-medium">Email Lama:</p>
                    <span class="font-bold text-center" style="margin-right: 900px; color: #006633;">{{ $old_email ?? '-' }}</span>
                    
                    <label for="email" class="text-sm font-medium text-green-700">Email Baru</label>
                    <input type="text" id="email" wire:model="email" 
                        class="form-control @error('email') is-invalid @enderror w-full max-w-xs rounded-lg border border-gray-300 bg-white focus:ring-green-500 focus:border-green-500 p-2.5">
                </div>
                @error('email')
