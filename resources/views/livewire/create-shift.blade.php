<div>
    <div class="flex justify-between items-center mb-5">
        <h1 class="text-2xl font-bold text-success-900">Tambah Data Shift</h1>
        ...
        <div class="grid grid-cols-2 gap-4 bg-success-100 border border-success-200 rounded-lg shadow-lg p-6">
            ...
            <div class="form-group col-span-2">
                ...
                <input type="text" id="nama_shift" wire:model.live="nama_shift"
                    class="form-control @error('nama_shift') is-invalid @enderror mt-1 block w-full rounded-lg border border-gray-300 bg-white focus:ring-success-500 focus:border-success-500 p-2.5" />
                @error('nama_shift')
                    <span class="text-danger text-sm">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="form-group col-span-2">
                ...
                <input type="time" id="jam_masuk" wire:model.live="jam_masuk"
                    class="form-control @error('jam_masuk') is-invalid @enderror mt-1 block w-full rounded-lg border border-gray-300 bg-white focus:ring-success-500 focus:border-success-500 p-2.5" />
                @error('jam_masuk')
                    <span class="text-danger text-sm">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="form-group col-span-2">
                ...
                <input type="time" id="jam_keluar" wire:model.live="jam_keluar"
                    class="form-control @error('jam_keluar') is-invalid @enderror mt-1 block w-full rounded-lg border border-gray-300 bg-white focus:ring-success-500 focus:border-success-500 p-2.5" />
                @error('jam_keluar')
                    <span class="text-danger text-sm">{{ $message }}</span>
                @enderror
            </div>
