<div>
    <div class="flex justify-between items-center mb-5">
        <h1 class="text-2xl font-bold text-green-700">Edit Username</h1>
    ...
    <form wire:submit.prevent="updateUsername">
        <div class="grid grid-cols-2 gap-4 bg-green-100 border border-green-200 rounded-lg shadow-lg p-6">
            <div class="form-group col-span-2">
                <p class="text-green-700 font-medium" style="margin-bottom: 20px;">Silahkan Masukan Username Anda yang Baru
                </p>

                <div class="grid grid-cols-2 gap-4 items-center">
                    <p class="text-green-700 font-medium">Username Lama:</p>
                    <span class="font-bold text-center"
                        style="margin-right: 900px; color: #006633;">{{ $old_username ?? '-' }}</span>

                    <label for="username" class="text-sm font-medium text-green-700">Username Baru</label>
                    <input type="text" id="username" wire:model="username"
                        class="form-control @error('username') is-invalid @enderror w-full max-w-xs rounded-lg border border-gray-300 bg-white focus:ring-green-500 focus:border-green-500 p-2.5">
                </div>
                @error('username')
