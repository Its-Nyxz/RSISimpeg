<div>
    <form wire:submit.prevent="updateMasaKerja">
        ...
        <div class="grid grid-cols-2 gap-4 bg-green-100 border border-green-200 rounded-lg shadow-lg p-6">
            ...
            <div class="form-group col-span-2">
                <label for="point" class="block text-sm font-medium text-green-900">Point Masa Kerja</label>
                <input type="text" id="point" wire:model="point" 
                    class="form-control @error('point') is-invalid @enderror mt-1 block w-full rounded-lg border border-gray-300 bg-white focus:ring-green-500 focus:border-green-500 p-2.5">
                @error('point')
                    <span class="text-danger text-sm">{{ $message }}</span>
                @enderror
            </div>
