<div class="container mx-auto py-6">
    <div class="flex justify-between items-center mb-5">
        <h1 class="text-2xl font-bold text-success-900">Tambah Data Pendidikan</h1>
                ...
                @if (!empty($minimGolonganOptions))
                    <ul
                        ...
                        @foreach ($minimGolonganOptions as $golongan)
                            <li wire:click="selectGolongan('minim_gol', '{{ $golongan->id }}', '{{ $golongan->nama }}')"
                ...
                @if (!empty($maximGolonganOptions))
                    <ul
                        ...
                        @foreach ($maximGolonganOptions as $golongan)
                            <li wire:click="selectGolongan('maxim_gol', '{{ $golongan->id }}', '{{ $golongan->nama }}')"
                                class="p-2 cursor-pointer hover:bg-success-200">
                                {{ $golongan->nama }}
                            </li>
                        @endforeach
