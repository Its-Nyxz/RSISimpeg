    ...
    public function loadData()
    {
        ...
        for ($day = 1; $day <= $daysInMonth; $day++) {
            ...
            $this->items[] = [
                ...
                'hari' => Carbon::parse($date)->locale('id')->isoFormat('dddd'),
                'tanggal' => Carbon::parse($date)->translatedFormat('d F Y'),
                'jam_kerja' => implode('|', $jamKerjaList),
                'jam_lembur' => implode('|', $jamLemburList),
                'rencana_kerja' => implode('|', $rencanaKerjaList) ?: '-',
                'laporan_kerja' => implode('|', $laporanKerjaList) ?: '-',
                'laporan_lembur' => implode('|', $laporanLemburList) ?: '-',
                'feedback' => implode('|', $feedbackList) ?: '-',
                'is_holiday' => $this->isHoliday($date),
                'is_lembur' => $isLembur,
                'is_dinas' => $isDinas,
                'late' => $isLate,
